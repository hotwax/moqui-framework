...
| Rule 1 `NS_ORDER_PUSH_NS_FACILITY`: `facilityGroupId equals NETSUITE_FULFILLMENT`, `shipmentMethodTypeId not-equals POS_COMPLETED` (added `d31a239`, 11 Sep, Anil: "yes"), sort `priority, orderDate`; rule 2 `NS_ORDER_PUSH_POS_COMPLETED`: `shipmentMethodTypeId equals POS_COMPLETED`, sort `orderDate` | `G/data/NetSuiteConfigData.xml` section 13, `2f83de3`, `d31a239` | before the row: a store added to the group sent its counter sale M121727 through rule 1; after: rule 1 took its two shipping orders, M121727 went with rule 2; a mixed cart (one line at the counter, one at WH) goes with rule 1 on its WH row (`record/test-scenarios-record.md`, runs 6 and 8) |
...
| `delete#NetSuiteSalesOrder`: SuiteQL `select distinct tl.transaction as id, t.type, t.tranid from transactionline tl join transaction t on t.id = tl.transaction where tl.createdfrom = <id>` first (the id in the text under `text-digits`; the sandbox answers 400 INVALID_CONTENT to a bound `?` param); any row: `linkedTransactions` out, error naming them, no DELETE; else `DELETE record/v1/salesOrder/{id}`; 204 deleted; 404 `notFound` with a message, no error | connector `0f19c4d`, `57c185b` on #398 | 70682121 (billed) refused before any DELETE, five named (IF5826393, CD5093611, CD5093612, RMA202717, INV6835006); 70687129 deleted, then 404; `70682121; drop` rejected by the validation. Then all 39 sandbox test orders since 8 Sep deleted (none linked), the 12 OMS `NETSUITE_ORDER_ID` rows expired, 11 orders re-brokered to WH. Earlier: | 70687228 (`HCTEST-COUNTRY-B`, pending) deleted; again, 404; 70682121 (`SO5726970`, billed) refused: "This transaction cannot be deleted because it is linked to by one or more transactions", still there. What depends on an order: `select distinct tl.transaction, t.type, t.tranid from transactionline tl join transaction t on t.id = tl.transaction where tl.createdfrom = <id>`; for 70682121 a fulfillment, two deposits, an RMA and an invoice |
